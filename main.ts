// 石の状態: 0=なし, 1=黒, 2=白
type StoneState = 0 | 1 | 2;
type Color = 1 | 2;

const stoneStateList: StoneState[] = [];
const stage = document.getElementById("stage") as HTMLElement;
const squareTemplate = document.getElementById("square-template") as HTMLElement;
const passButton = document.getElementById("pass") as HTMLButtonElement;

class TurnManager {
  currentColor: Color;
  currentTurnText: HTMLElement;

  constructor() {
    this.currentColor = 1;
    this.currentTurnText = document.getElementById("current-turn") as HTMLElement;
  }

  changeTurn(): void {
    this.currentColor = (3 - this.currentColor) as Color;
  ...
  highlightAvailableMoves(): void { // 強調表示
...
class ReversiLogic {
  static getReversibleStones(idx: number, currentColor: Color): number[] {
    const squareNums = [
      7 - (idx % 8),
...
class Board {
  ...
  constructor(stage: HTMLElement, template: HTMLElement, clickHandler: (index: number) => void) {
    this.stage = stage;
  ...
  createSquares(): void {
    for (let i = 0; i < 64; i++) {
      const square = this.template.cloneNode(true) as HTMLElement;
      square.removeAttribute("id");
      this.stage.appendChild(square);

      const stone = square.querySelector(".stone") as HTMLElement;

      let defaultState: StoneState;
...
class Game {
  ...
  onClickSquare(index: number): void {
    ...
    document
      ...
      ?.setAttribute("data-state", String(this.turnManager.currentColor));
    ...
    reversibleStones.forEach((key) => {
      ...
      document.querySelector(`[data-index='${key}']`)?.setAttribute("data-state", String(this.turnManager.currentColor));
    });
